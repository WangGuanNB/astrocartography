...
const Starfield: React.FC<StarfieldProps> = ({
  ...
  bgColor = 'rgba(15,15,35,1)', // 🎨 恢复原来的深蓝色背景
  mouseAdjust = true, // 启用鼠标交互
  tiltAdjust = false,
  easing = 6, // 增加缓动，让移动更平滑
  clickToWarp = false,
  hyperspace = false,
  warpFactor = 10,
  opacity = 0.15, // 稍微增加透明度
  speed = 0.8, // 稍微减慢速度，更优雅
  quantity = 400, // 减少星星数量，提升性能
}) => {
  ...
  const sd = useRef({
    ...
    y: 0,
    z: 0,
    star: { colorRatio: 0, arr: [] as (number | boolean)[][] }, // 修复类型定义
    prevTime: 0,
  });
  ...
  const draw = () => {
    ...
    sd.current.star.arr.forEach(star => {
      if (
        ...
        star[7] === 1 // 检查是否为 1（相当于 true）
      ) {
        ctx.lineWidth = (1 - sd.current.star.colorRatio * (star[2] as number)) * 2;
        ctx.beginPath();
